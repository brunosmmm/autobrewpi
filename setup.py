...
class ABPIInstall(SetuptoolsInstall):
    ...
    def run(self):
        ...
        compile_lcd = subprocess.Popen(['make'], stdout=subprocess.PIPE,
                                       cwd='lcd')
        if compile_lcd.wait() != 0:
            raise RuntimeError('could not compile lcd module')
        print('Compiling user I/O server...')
        compile_io = subprocess.Popen(['make'], stdout=subprocess.PIPE,
                                      cwd='io')
...
setup(
    ...
    package_dir={'': '.'},
    package_data={'abpi': ['data/fonts/*', 'lcd/screen.so']},
