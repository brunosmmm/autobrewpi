from abpi.gadget.hbusjson import HbusClient
from abpi.util.thread import StoppableThread

# from abpi.gadget.vspacerpc import VSpaceRPCController
import time
...
class VSpacePortDescriptor(object):
    def __init__(
        self, direction, port_id, linked_instance, instance_port_name
    ):
        self._dir = direction
        self._pid = port_id
...
class GadgetVariableSpace(StoppableThread):
    ...
    def __init__(self, gadget_uid, scan_interval=1.0, hbus_timeout=1.0):
        ...
        self.logger.debug("waiting for HBUS master to be ready")
        tries = 1
        master_state = None
        try:
        ...
        while master_state != 5:
            ...
            try:
                ...
                tries += 1
            if tries > 3:
                self.logger.error("HBUS master not ready or cannot connect")
                self._hcli.stop()
    ...
    def _parse_objects(self):
        ...
        for obj in obj_list:
            ...
            self._varspace[obj_name] = GadgetVariableProxy(
                obj_idx, can_read, can_write
            )
            # create port aliases
            if can_write:
                port_id = self._new_port_added()
                port = VSpacePortDescriptor(
                    "input", port_id, "gadget", obj_name
                )
            ...
            if can_read:
                port_id = self._new_port_added()
                port = VSpacePortDescriptor(
                    "output", port_id, "gadget", obj_name
                )
                self._gadget_space_port_matrix[port_id] = port
    ...
    def _scan_variable(self, var_name, rd_callback):
        ...
        if var._rd:
            try:
                if var._read_pending:
                    return
                self._hcli.read_slave_object(
                    self._gaddr, var._idx, False, rd_callback, var_name
                )
    ...
    def __setattr__(self, name, value):
        if self.__initialized:
            if name in list(self.__getattribute__("_varspace").keys()):
                self._write_queue.appendleft(
                    GadgetVariableRequest(name, value)
                )
                return
    ...
    def register_driver(self, driver_class, instance_name, **kwargs):
        ...
        self.logger.debug(
            "registering instance {} with {} inputs and {} outputs".format(
                instance_name,
    ...
    def _process_driver_ports(self, driver_object):
        ...
        for inp_name, inp_obj in list(driver_object._inputs.items()):
            ...
            port_id = self._new_port_added()
            self.logger.debug(
                "assigning port number {} to port {}.{}".format(
                    port_id, driver_object.get_instance_name(), inp_name
                )
            )
            port = VSpacePortDescriptor(
                "input", port_id, driver_object.get_instance_name(), inp_name
            )
        ...
        for out_name, out_obj in list(driver_object._outputs.items()):
            ...
            self.logger.debug(
                "assigning port number {} to port {}.{}".format(
                    port_id, driver_object.get_instance_name(), out_name
                )
            )
            port = VSpacePortDescriptor(
                "output", port_id, driver_object.get_instance_name(), out_name
            )
    ...
    def connect_pspace_ports(self, port_from, port_to):
        self.logger.debug(
            "connecting port {} to port {}".format(port_from, port_to)
        )
        ...
        if space_from == self._process_space_port_matrix:
            self._drivers[
                space_from[port_from].get_linked_instance_name()
            ].port_connected(port_from, port_to)
        if space_to == self._process_space_port_matrix:
            self._drivers[
                space_to[port_to].get_linked_instance_name()
            ].port_connected(port_to, port_from)
        ...
        if output_port is not None:
            if output_port.get_linked_instance_name() == "gadget":
                value = self._varspace[
                    output_port.get_linked_port_name()
                ].get_value()
            else:
                value = (
                    self._drivers[output_port.get_linked_instance_name()]
                    ._outputs[output_port.get_linked_port_name()]
                    .get_stored_value()
                )
            ...
            if input_port is not None:
                if value is not None:
                    if input_port.get_linked_instance_name() == "gadget":
                        self._varspace[
                            input_port.get_linked_port_name()
                        ].set_value(value)
                    else:
                        self._drivers[
                            input_port.get_linked_instance_name()
                        ].update_local_variable(
                            input_port.get_linked_port_name(), value
                        )
    ...
    def is_port_connected(self, port_id):
        if port_id in self._gadget_space_port_matrix:
            return (
                len(self._gadget_space_port_matrix[port_id].get_connected_to())
                > 0
            )
        elif port_id in self._process_space_port_matrix[port_id]:
            return (
                len(
                    self._process_space_port_matrix[port_id].get_connected_to()
                )
                > 0
            )
    ...
    def trigger_output_change(self, instance_name, output_variable, new_value):
        ...
        self.logger.debug(
            'instance "{}" registered a change'
            ' in output "{}", propagating...'.format(
                instance_name, output_variable
            )
        )
    ...
    def _propagate_value(self, port_global_id, new_value):
        ...
        for connected_to in space[port_global_id].get_connected_to():
            ...
            if connected_to in self._process_space_port_matrix:
                self.logger.debug(
                    "propagating value through "
                    "process space port {}".format(connected_to)
                )
                port_descriptor = self._process_space_port_matrix[connected_to]
                self._drivers[
                    port_descriptor.get_linked_instance_name()
                ].update_local_variable(
                    port_descriptor.get_linked_port_name(), new_value
                )
    ...
    def get_connection_matrix(self):
        ...
        for port_id, port_object in list(
            self._process_space_port_matrix.items()
        ):
            ret["process"][port_id] = port_object._connection_list
        for port_id, port_object in list(
            self._gadget_space_port_matrix.items()
        ):
    ...
    def get_port_info(self, port_id):
        ...
        return {
            "direction": pspace[port_id].get_direction(),
    ...
    def call_driver_method(self, instance_name, method_name, **kwargs):
        return self._call_driver_method(
            "local", instance_name, method_name, **kwargs
        )

    def _call_driver_method(
        self, source, instance_name, method_name, **kwargs
    ):
        if instance_name in self._drivers:
            return self._drivers[instance_name].extern_method_call(
                source, method_name, **kwargs
            )
        else:
            raise KeyError("Invalid instance name: {}".format(instance_name))
    ...
    def rpc_request(self, request, *args, **kwargs):
        if request == "portlist":
            return {
                "gadget": list(self._gadget_space_port_matrix.keys()),
    ...
    def get_current_value(self, port_id):
        ...
        if port_id in self._gadget_space_port_matrix:
            var_name = self._gadget_space_port_matrix[
                port_id
            ].get_linked_port_name()
            return self._varspace[var_name].get_value()
        elif port_id in self._process_space_port_matrix:
            inst_name = self._process_space_port_matrix[
                port_id
            ].get_linked_instance_name()
            var_name = self._process_space_port_matrix[
                port_id
            ].get_linked_port_name()
            ...
            try:
                ...
                self.logger.warning(
                    "get_current_value: no such port: {}.{}".format(
                        inst_name, var_name
                    )
                )
                return None
    ...
    def run(self):
        ...
        while True:
            ...
            if datetime.now() > self._last_scan + timedelta(
